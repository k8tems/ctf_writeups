...
function log_code(addr, op, info)
{
    console.log(to_hex(addr).padEnd(4) + " " + op.padEnd(4) + " " + info)
}
    ...
    run()
    {
        ...
        switch(this.fetch_uint8())
        {
            case 2:  // substitute var
                ...
                log_code(cur_ip, "MOV", var_30_.repr() + "," + var_32_.repr())
                break;
            case 10: // substitute var1 to v2 as uint32
                var var_30_ = this.create_var();
                var var_32_ = this.create_var();
                var_30_.set_value(var_32_.get_value()>>>0);
                log_code(cur_ip, "MOVU", var_30_.repr() + "," + var_32_.repr());
                break;
            case 1:  // push var
                var var_30_ = this.create_var();
                log_code(cur_ip, "PUSH", var_30_.repr())
                this.push_value(var_30_.get_value());
                break;
            case 14: // pop to var
                var var_30_ = this.create_var();
                var_30_.set_value(this.pop_value());
                log_code(cur_ip, "POP", var_30_.repr())
                break;
            case 4: // call var
                var var_30_ = this.create_var();
                log_code(cur_ip, "CALL", var_30_.repr())
                break;
            ...
            case 8:  // substitute ip with var
                var var_30_ = this.create_var();
                log_code(cur_ip, "JMP", var_30_.repr())
                break;
            case 9:  // jump if overflow_flag is false
                var var_30_ = this.create_var();
                log_code(cur_ip, "JNO", var_30_.repr());
                break;
            case 7:  // jump if equal_flag is false
                var var_30_ = this.create_var();
                log_code(cur_ip, "JNE", var_30_.repr());
                break;
            case 16: // jump if equal_flag is true
                var var_30_ = this.create_var();
                log_code(cur_ip, "JE", var_30_.repr());
                break;
            case 18:  // shift as uint32?
                var var_30_ = this.create_var();
                var var_32_ = this.create_var();
                log_code(cur_ip, "SHRU", var_30_.repr() + "," + var_32_.repr());
                break;
            case 11:  // xor var
                var var_30_ = this.create_var();
                var var_32_ = this.create_var();
                log_code(cur_ip, "XOR", var_30_.repr() + "," + var_32_.repr());
                break;
            case 17: // and vars
                var var_30_ = this.create_var();
                var var_32_ = this.create_var();
                log_code(cur_ip, "AND", var_30_.repr() + "," + var_32_.repr());
            ...
            case 13: // or vars
                var var_30_ = this.create_var();
                var var_32_ = this.create_var();
                log_code(cur_ip, "OR", var_30_.repr() + "," + var_32_.repr());
                var_30_.set_value(var_30_.get_value() | var_32_.get_value());
                break;
            case 5: // test and set difference of uint vars in former var
                var var_30_ = this.create_var();
                var var_32_ = this.create_var();
                log_code(cur_ip, "ADD", var_30_.repr() + "," + var_32_.repr());
                var var_31_ = var_30_.get_value() + var_32_.get_value();
                this.overflow_flag = new Boolean(var_31_>>32);
                var_30_.set_value(var_31_);
                break;
            case 3:  // test and set difference of vars in former var
                var var_30_ = this.create_var();
                var var_32_ = this.create_var();
                log_code(cur_ip, "SUB", var_30_.repr() + "," + var_32_.repr());
                // var30 - (-var32) ??
            ...
            case 6:  // test equality of vars; test difference of vars
                var var_30_ = this.create_var();
                var var_32_ = this.create_var();
                log_code(cur_ip, "TEST", var_30_.repr() + "," + var_32_.repr());
...
function GetFlag(password)
{
    ...
    var vm = new VirtualDisassembler(code);
